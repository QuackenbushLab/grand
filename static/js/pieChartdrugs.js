$(document).ready(function() {
    ...
    var data3 = rows.map(function(row) {
      return row.fields['pert_idose'];
  })
  
  var dataETH = rows.map(function(row) {
      return row.fields['pert_itime'];
  })
  
  var dataAL = rows.map(function(row) {
    return row.fields['sample_type'];
})
  
  var dataStage = rows.map(function(row) {
    return row.fields['donor_sex'];
  })

  var dataVital = rows.map(function(row) {
    return row.fields['donor_ethnicity'];
})
  ...
  function foo(arr) {
    ...
    var pieChartDataVital = {
      ...
      var table = $('#drugstable2').DataTable({
        "order": [[ 1, "asc" ]],
